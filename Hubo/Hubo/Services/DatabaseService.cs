        ...
        internal async Task<bool> StopDrive(int hubo, string note, string location)
        {
            ...
            using (UserDialogs.Instance.Loading(Resource.EndingDrive, null, null, true, MaskType.Gradient))
            {
                ...
                while (!correctHubo)
                {
                    ...
                    {
                        if (await UserDialogs.Instance.ConfirmAsync("Starting Odometer: " + drive.StartHubo + " \nis HIGHER than \nEnding Odometer: " + hubo, "Re-Enter Odometer", "Re-Enter odometer", "This is the correct odometer"))
                        {
                            hubo = await HuboPrompt(hubo);
                        }
                    }
                    ...
                    {
                        await UserDialogs.Instance.AlertAsync(Resource.Alert, Resource.InvalidHubo, Resource.GotIt);
                        return false;
                    }
                    else
                    {
